...
class DistillationStrategy(TrainingStrategy):
    ...
    ValueError
        ...
        on-policy mode, additionally if the run is sized in epochs rather than
        steps, if the propagator holds neither the student nor a model
        composing it, if ``replay_ratio`` is ``0``, if a ratio below ``1`` is
        paired with no ``reference_dataset``, if a ratio of ``1`` is paired
        with one, if the ratio and ``batch_size`` together allocate no samples
        to one mixture source, if ``replay_device`` names a device the
        ``reference_dataset`` does not emit on, if the ``reference_dataset``
        carries fields the labeling hook strips from every generated frame, if
        the propagator's scorer declares a field outside the ``teacher_*``
        namespace, or if that scorer's known fields and ``reference_dataset``
        do not carry the same teacher fields.
