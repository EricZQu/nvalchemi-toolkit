...
from nvalchemi.training import EnergyMSELoss, OptimizerConfig, TrainingStage
        ...
        [
            AtomicData(
                positions=torch.randn(
                    4, 3, generator=torch.Generator().manual_seed(700 + index)
                ),
...
class TestAdvancedObjectivesOnCuda:
    ...
    def test_embedding_objective_trains_on_cuda(self) -> None:
        ...
        projector = EmbeddingProjector(_STUDENT_WIDTH, _TEACHER_WIDTH)
        strategy = _make_embedding_strategy(
            projector=projector, devices=[torch.device("cuda")]
        )
    ...
    def test_hessian_objective_trains_on_cuda(self) -> None:
        ...
        strategy = _make_hessian_strategy(devices=[torch.device("cuda")])
